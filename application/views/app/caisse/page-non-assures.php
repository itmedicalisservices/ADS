...
<?php 
	if($user->flt_sLib == 'Administration' || $user->flt_sLib == 'Comptabilite') {
		...
		$liste = $this->md_patient->liste_facture_non_assure2($this->md_config->get_session()); 
	}
    ...
    <div class="container-fluid">
		...
		<div class="row clearfix">
			<div class="col-lg-12 col-md-12 col-sm-12">
				<div class="card">
					<div class="header">
						<h2>Recherche facture par numéro</h2>
					</div>
					<div class="body">
						<form id="form-rech-fac">
							<div class="row clearfix">
								<div class="col-sm-5">
									<div class="form-group">
										* N° Facture<input style="border:1px solid black" type="text" name="rech" class=" form-control obligatoire" placeholder="* Saisissez le numéro de facture">
									</div>
								</div>
								<div class="col-sm-2">
								<br><br>
									<button type="submit" class="btn btn-raised bg-blue-grey" id="rechFac">Recherche</button>
								</div>
							</div>
						</form>
					</div>
						...
						<table id="example" class="table  table-hover" style="font-size:12px">
							...
							<?php $Total3 +=$l->fac_iReste; ?>
								<tr align="center">
									...
									<td>
										<?php if(is_null($l->mod_id)){echo '0';}else{ $mod = $this->md_parametre->recup_motif_reduction($l->mod_id); echo $mod->mod_iTaux; $Total1 +=$mod->mod_iTaux;}; ?> %
									...
									<td>
										...
										<?php }else{ ?>
										<span class="label label-warring">avancé</span>
										<?php } ?>
									</td>
									<td>
									...
									<?php }else{?>
										...
										<?php }else{?>
											<?php if(is_null($l->fac_iStAnnul)){?>
												<?php if($l->fac_iSta == 1){?>
														<a onClick="return confirm('Êtes-vous sûr de vouloir annuler cette facture ?')" href="<?php echo site_url("facture/annuler_facture/".$l->fac_id);?>" class="text-danger" title="Annuler cette facture ?" ><i class="fa fa-times" style="font-size:20px"></i></a>
												<?php }else{?>
