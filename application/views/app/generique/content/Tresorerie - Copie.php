...
<?php $sommecumul = $this->md_patient->total_encaissee($this->md_config->get_session()); ?>
<?php $sommecumulannulation = $this->md_patient->total_annulation_caissee($this->md_config->get_session()); ?>

<?php $actesdiv = $this->md_parametre->liste_frais_divers_actifs(); ?>
<?php $locataire = $this->md_parametre->liste_locataire_actifs(); ?>
<?php $mouvements = $this->md_patient->liste_tout_mouvement_caisse($this->md_config->get_session()); ?>
<?php $datedebut = $this->md_patient->liste_tout_mouvement_caisse_datedebut($this->md_config->get_session()); ?>
<?php $listeactesdiv = $this->md_parametre->liste_acts_divers_actifs(); ?>
<?php $listepatient = $this->md_patient->liste_patient_100(); ?>
<?php 
	$articleParPage = 100;
                ...
                <div class="row clearfix">
					<div class="col-lg-12 col-md-12 col-sm-12">
						<div class="card">
							...
							<?php  //var_dump($this->md_patient->liste_element_caisse(date("y-m-d h:i:s")));?>
        ...
        <div class="modal-content">
                <div class="row clearfix">
                    <div class="col-lg-12 col-md-12 col-sm-12">
						<div class="card">
							<div class="header">
								<h2>Liste Total des Patients (<?php echo $this->md_patient->nb_patients()->nb;?>)</h2></br></br>
								<input type="text" name="search" id="search2" placeholder="Recherche ..." style="width:30%;padding-left:1%;margin-left:1%" oninput="recherche1()"/>
...
<div class="modal fade" id="modalPaye" role="dialog">
    <div class="modal-dialog modal-lg" role="document" style="margin-top:5px;">
        <div class="modal-content">
			...
			<form action="<?php echo site_url('caisse/ajoutFactureCaisse');?>" method="POST" id="form-caisse" style="border:1px solid black; margin-top:-30px">
...
<div class="modal fade" id="mdModal" tabindex="-1" role="dialog">
    ...
    <script>
		...
		function recherche1(){
			...
			if($("#search2").val() ==""){
				...
				.done(function(retour){
					 //alert(retour);
					if(retour == ""){
						$(".listePat").html('<tr><td colspan="6"><em>Aucun patient trouver</em></td></tr>');
					}else{
		...
		function recherche_element_caisse1(){
			
			if($("#search").val() ==""){
				$.ajax({
					type:"POST",
					url: recherche_element_caisse_100,
				...
				.done(function(retour){
					 //alert(retour);
					if(retour == ""){
						$(".listeElement").html('<tr><td colspan="6"><em>Aucun patient trouver</em></td></tr>');
					}else{
						$(".listeElement").html(retour);
					}
		...
		function recherche_element_caisse(){
			//alert($("#search").val());
			if($("#search").val() !=""){
				$.ajax({
					type:"POST",
					url: recherche_element_caisse2,
					data:"search="+$("#search").val(),
					async:true,
					error:function(xhr, status, error){
						alert(1);
