...
class Md_pharmacie extends CI_Model {
	...
	protected $tableDes = "armee.t_destockage_des";
	protected $tableFac = "armee.t_facture_fac";
	protected $tableElf = "armee.t_elements_facture_elf";
	protected $tableClt = "armee.t_client_clt";
	protected $tableBph = "armee.t_bon_pharmacie_bph";
	protected $tableCmd = "armee.t_commande_cmd";
	protected $tableDcm = "armee.t_detail_commande_dcm";
	protected $tablePer = "armee.t_personnel_per";
	protected $tableDac = "armee.t_detail_achats_dac";
	...
	public function recup_fictif(){
		return $this->db->select('fic_sProd, fic_iPu, ach_id, COUNT(fic_sProd) as total')
				->group_by('fic_sProd')
				->group_by('fic_iPu')
				->group_by('ach_id')
				// ->order_by('total', 'desc')
	...
	public function bilan_medicament_stock()
	{
		...
		return $this->db->select('med_id, SUM(ach_iPrixTotalAchat) as achat, SUM(ach_iQte) as quantite, SUM(ach_iPrixTotalVente) as vente')
				->group_by('med_id')
				->get($this->tableAch)->result();
	}	
	...
	{
		return $this->db->select('MONTH(CONVERT(date,fac_dDatePaie)) AS mois, SUM(fac_iMontantPaye) AS montant, COUNT(fac_id) AS nb')
				->where("fac_sObjet","Vente médicament")
				->like('fac_dDatePaie',$date)
				->group_by('MONTH(CONVERT(date,fac_dDatePaie))')
				->order_by('MONTH(CONVERT(date,fac_dDatePaie))')
	...
	public function recup_annee_vente()
	{
		...
		return $this->db->select('DISTINCT YEAR(CONVERT(date,pro_dDateDestock)) AS dates')
				->group_by('pro_dDateDestock')
				->get($this->tablePro)->result();
	}	
	...
	//RABY
	public function rupture($donnees){
		return  $this->db->insert($this->tableHis,$donnees);
	}	

	public function maj_rupture($donnees,$id){
		return $this->db->where("ach_id",$id)->update($this->tableHis,$donnees);
	}
	public function verif_stock_restant($med){
		return $this->db->where("ach_id",$med)->get($this->tableAch)->row();
	}	
	//RABY
	...
	//RABY
	public function nb_jours_rupture($id,$dateDepart,$dateFinal)
	{
		return $this->db
		->join($this->tableAch, $this->tableHis.'.ach_id='.$this->tableAch.'.ach_id','inner')
		->join($this->tableMed, $this->tableMed.'.med_id='.$this->tableAch.'.med_id','inner')
		->where($this->tableMed.".med_id",$id)
		->where($this->tableHis.".his_dDateDebutRup >=",$dateDepart)
		->where($this->tableHis.".his_dDateDebutRup <=",$dateFinal)
		->get($this->tableHis)->result();
	}
	 //RABY
