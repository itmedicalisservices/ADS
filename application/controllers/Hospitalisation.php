...
class Hospitalisation extends CI_Controller {
	...
	{
		$this->load->view('app/medecin_generaliste/page-liste-hospitalisation');
	}
	...
	public function patient_hospitalise($id)
	{
		...
		$this->load->view('app/medecin_generaliste/page-patient-hospitalise',array("h"=>$id));
	}
	...
	public function deces_nouveauNe($id)
	{
		date_default_timezone_set('Africa/Brazzaville');

		$donnees = array(
			"nne_dDateDeces"=>date("Y-m-d")                                
		);
		
		$this->md_patient->deces_nouveauNe($id,$donnees);
	}
