...
class Caisse extends CI_Controller {
					...
					<div class="card all-patients">
						<div class="body">
							...
							<div class="row">
								...
								<div class="col-md-7 col-sm-7 m-b-0">
									<input type="number" class="obligatoire" name="montant" style="width:88%;padding:2px;" placeholder="Mettez la valeur en FCFA *"/> FCFA
								</div>
							</div>
							<div class="row">
							...
							<div class="row">
								<div class="col-md-3 col-sm-3 m-b-0">
									<p><b>Médecin Prescript.</b></p> 
								</div>
								<div class="col-md-7 col-sm-7 m-b-0">
									<select id="medPrst" class="medPrst" name="prescripteur" style="width:100%;padding:5px;z-index:99999">
										';
											$listepms = $this->md_parametre->liste_prescripteur_actifs();
											//$listepms = $this->md_personnel->liste_personnel_medical_sante();
											echo '<option value="">-- Sélectionner le médecin prescripteur --</option>';
											foreach($listepms AS $l){
												echo '<option value="'.$l->pre_id.'">'.$l->pre_sNom.' '.$l->pre_sPrenom.'</option>';
											}
											echo '
									</select>              
								</div>
	...
	public function ajoutFactureCaisse()
	{
		...
		for($i=0;$i<count($data["acm"]) AND count($data["duree"]);$i++){
			...
			$expiration = date("Y-m-d H:i:s",$maDate);/*après*/
		...
		//RABY
		if($data["montant"]==""){$montant=0;}else{$montant=$data["montant"];}
		//RABY
		if($data["montantReduc"]==0){
			//$paye = $data["total"] - $data["charge"]; //calcule le montant payé par le client
			$reduct = NULL; 
		}else{
			$reduct = $data["total"] - $data["charge"] - $data["montantReduc"];
			//$paye = $data["montantReduc"] - $data["charge"]; //calcule le montant payé par le client
		}
		
		$patient = $this->md_patient->recup_patient($data["patient"]);
		
		if($data["prescripteur"] == ""){$Prsct = NULL;}else{$Prsct = $data["prescripteur"];}
		...
		$donneeFac = array(
			...
			"fac_iMontantPaye"=>$montant,
			"fac_iMontant"=>$data["total"],
			"fac_iMontantAss"=>$data["charge"],
			"fac_iReste"=>$data["total"]-($data["charge"] + $montant),
			"fac_dDatePaie"=>date("Y-m-d"),
		...
		$insertFac = $this->md_patient->ajout_facture($donneeFac);
				...
				$qrcode = $this->md_config->QRcode($numero, $patientconcerne,date("Y-m-d"),$data["total"],$montant);
		...
		for($i=0;$i<count($data["acm"]) AND count($data["somme"]) AND count($data["lac"]);$i++){
			...
			$donneesElt = array(
				...
				"rub_id"=>$recupQpt->rub_id,
				"fac_id"=>$insertFac,	
				"per_iElf"=>$this->md_config->get_session(),
	...
	public function closecaisse()
	{
		...
		if(empty($data)){
			...
			$user = $this->md_connexion->personnel_connect();
			$connectMail = $this->md_connexion->se_connecter_email($user->per_sEmail,$pwd);
			$sommecumul = $this->md_patient->total_encaissee($this->md_config->get_session());
	...
	public function opencaisse()
	{
		...
		if(empty($data)){
			...
			// return;
			if(!is_null($connectMail)){
				// $this->session->set_userdata(array("itmedicalis"=>$connectMail->per_id));
...
// return;
	...
	public function ajoutCession()
	{
		...
		if(empty($data)){
			...
			$user = $this->md_connexion->personnel_connect();
			$sommecumul = $this->md_patient->total_encaissee($this->md_config->get_session());
			$sommecumulannulation = $this->md_patient->total_annulation_caissee($this->md_config->get_session());
			// echo var_dump($data);
			// return;
			if($data['espece2'] > 0){
	...
	public function ajoutPassation()
	{
		...
		$caissier = $this->md_patient->last_mouvement_caissier_passation($this->md_config->get_session());

		$donneespassation = array(
			"pas_iSta"=>0,
			"pas_iAuteur"=>$this->md_config->get_session(),
		...
		$data = $this->input->post();
			...
			$modif=$this->md_patient->maj_annulation_cumul_caisse($annuleCumulusercourant,$this->md_config->get_session());	/*Met le cumul de l'user à zero*/
			/*
			$donneesdeconnectusercourant = array(
				"per_iCnx"=>NULL
			);
			$modif=$this->md_personnel->modifier_personnel($donneesdeconnectusercourant,$this->md_config->get_session());	/*Ferme la caisse de user courant*/
			...
			$donneeFac2 = array(
				...
				"pat_id"=>0,
				"per_id"=>$data["recept"],
				"recp_iPer"=>$this->md_config->get_session(),
	...
	public function recupMvtCaisse()
	{
		...
		$liste  = $this->md_patient->liste_mouvement_caisse_facture($this->md_config->get_session(), $premier, $dernier, $data["acte"], $data["jrnal"]);
	// var_dump($liste);
		echo '	
						...
						<div class="table-responsive">
							<table id="" class="table table-bordered table-striped table-hover" style="font-size:12px">
								<thead>
									<tr align="center">
										<td>
											<a title="Imprimer le journal" href="'; echo site_url("impression/mouvement_caisse_facture/".$this->md_config->get_session()."/".$premier."/".$dernier."/".$data["acte"]."/".$data["jrnal"]); echo'" class="btn btn-sm btn-raised bg-blue-grey" style="color:white;font-size:13px"><i class="fa fa-print"></i>  imprimer journal</a>
										</td>
							...
							<?php $annulation = $this->md_parametre->recup_liste_annulation_caissier($this->md_config->get_session(), $premier, $dernier);?>
							...
							<?php $cession = $this->md_parametre->recup_liste_cession_caissier($this->md_config->get_session(), $premier, $dernier);?>
							<?php $passation = $this->md_parametre->recup_liste_passation_caissier($this->md_config->get_session(), $premier, $dernier);?>
	...
	public function recupMvtCaisseActe()
	{
		...
		$liste  = $this->md_patient->liste_facture_acte($this->md_config->get_session(), $premier,$dernier);
						...
						<div class="table-responsive">
							<table id="" class="table table-bordered table-striped table-hover" style="font-size:12px">
								<thead>
									<tr align="center">
										<td>
											<a title="Imprimer tous les mouvements" href="'; echo site_url("impression/mouvement_caisse_acte/".$this->md_config->get_session()."/".$premier."/".$dernier); echo'" class="btn btn-sm btn-raised bg-blue-grey" style="color:white;font-size:13px"><i class="fa fa-print"></i>  mouvements (';echo count($liste);echo')</a>
										</td>
	...
	public function valide_appro($id){
		...
		if(!isset($id)){
			...
			$donnees = array(
				...
				"apr_iPerVal"=>$this->md_config->get_session(),
	...
	public function rechFacNum()
	{
		$data = $this->input->post();
		$chaineOne = substr(trim($data['rech']),4);
		$chaineTwo = substr($chaineOne,0, -8);
		$facture = $this->md_patient->recherche_facture($chaineTwo);
			{ ?>
			
			<?php //var_dump($chaineOne);?>
			<?php //var_dump($chaineTwo);?>
			
			<?php if(is_null($facture) || empty($facture)){?>
				<em>Aucun resultat trouvé !</em>
			<?php }else{?>
					<div class="body table-responsive"> 
						<table id="" class="table table-bordered table-striped table-hover" style="font-size:12px">
							...
							<tbody>
								<tr align="center">
									<td>
										<?php  $pat = $this->md_patient->recup_patient($facture->pat_id); echo $pat->pat_sNom; ?> <?php echo $pat->pat_sPrenom; ?>
									</td>
									<td>
										<?php echo $facture->fac_iMontant; ?> <small>FCFA</small>
									</td>
									<td>
										<?php if(is_null($facture->mod_id)){echo '0';}else{ $mod = $this->md_parametre->recup_motif_reduction($facture->mod_id); echo $mod->mod_iTaux;}; ?> %
									</td>
									<td>
										<?php echo $facture->fac_iMontantPaye; ?> <small>FCFA</small>
									</td>
									<td>
										<?php echo $this->md_config->affDateFrNum($facture->fac_dDatePaie); ?>
									</td>								
									<td>
										<?php echo $facture->fac_sNumero; ?>
									</td>																		
									<td>
										<b><?php $pers = $this->md_personnel->recup_personnel($facture->per_id); echo $pers->per_sNom.' '.$pers->per_sPrenom; ?></b>
									</td>
									<td class="text-center">
										<?php if($facture->fac_iSta == 1){?>
												<a onClick="return confirm('Êtes-vous sûr de vouloir annuler cette facture ?')" href="<?php echo site_url("facture/annuler_facture/".$facture->fac_id);?>" class="text-danger" title="Annuler cette facture ?" ><i class="fa fa-times" style="font-size:20px"></i></a>
										<?php }else{?>
											<a onClick="return confirm('Êtes-vous sûr de vouloir restaurer cette facture ?')" href="<?php echo site_url("facture/restaure_facture/".$facture->fac_id);?>" class="text-success" title="Restaurer cette facture ?" ><i class="fa fa-history" style="font-size:20px"></i></a>
										<?php }?>
									</td>
								</tr>
							</tbody>
