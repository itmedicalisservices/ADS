...
class Laboratoire extends CI_Controller {
	...
	public function prelevement_tube($id)
	{
		...
		foreach($element AS $e){
			...
			$dataTube = array(
				...
				"tan_iPerNumero"=>$this->md_config->get_session(),
				"ela_id"=>$e->ela_id
			);
	...
	public function sortirAccessoire()
	{
		...
		$data = $this->input->post();
			...
			$donnees = array(
				"acs_iSta"=>1,
				"per_iAutorisant"=>$this->md_config->get_session(),
	...
	public function sortieReactif()
	{
		...
		if($update){
			$donnees = array(
				"sor_iSta"=>1,
				"res_id"=>$data['idRes'],
				"res_iDon"=>$this->md_config->get_session(),
				"res_iDest"=>$data['per'],
				"app_id"=>$data['app'],
	...
	public function ajoutRapport()
	{
		...
		if(empty($data)){
			...
			$donneesRapport = array(
				...
				"tan_iPerRapport"=>$this->md_config->get_session(),
                ...
                <div class="info-box-4 hover-zoom-effect" style="height:auto; width:100%">
                    <div class="content">
						...
						foreach($listeActeLabo AS $r){
							...
							echo '<div class="text">'.$r->lac_sLibelle.'  -> <span class="pull-right">'.count($nb).'</span></div>';
						}
                   echo '</div>
                </div>
            </div>';			
		...
		$lit = $this->md_laboratoire->liste_valeur_element_analyse($data["id"]);
			...
			echo '<div class="row">';
				...
				echo '<div class="col-md-12">';
					...
					echo '<textarea type="text" rows="5" id="edit" class=" obligatoire" name="contenu" style="width:100%"></textarea>';
				echo '</div>';
			echo '</div>';
		echo '</form>
		 <script>
