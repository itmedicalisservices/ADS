	...
	public function entreeStock()
	{
		...
		if(empty($data)){
			...
			if(!$verif){
				...
				$verifQte=$this->md_pharmacie->verif_stock_restant($verif->ach_id);
				if($verifQte AND $verifQte->ach_iQte <=0){
					$datarupture = array(
						'his_dDateFinRup'=>date("Y-m-d")
					);
					$rupture = $this->md_pharmacie->maj_rupture($datarupture,$verif->ach_id);
				}
				$donnees = array(
